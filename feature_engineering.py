...
class FantasyFeatureEngineer:
    ...
    def add_team_context(self):
        ...
        standings_copy = self.standings.copy()
        ...
        try:
            ...
            for _, row in standings_copy.iterrows():
            ...
            # Apply to game logs - need to match "Team.DALLAS_MAVERICKS" with "Dallas Mavericks"
            def match_team(game_team, season):
                # Convert "Team.DALLAS_MAVERICKS" to "Dallas Mavericks"
                if isinstance(game_team, str) and game_team.startswith('Team.'):
                    # Remove "Team." prefix and convert to title case
                    clean_name = game_team.replace('Team.', '').replace('_', ' ').title()
                    ...
                    for (mapped_team, mapped_season), is_cont in team_mapping.items():
                        if mapped_season == season and clean_name.lower() in mapped_team.lower():
                            return is_cont
